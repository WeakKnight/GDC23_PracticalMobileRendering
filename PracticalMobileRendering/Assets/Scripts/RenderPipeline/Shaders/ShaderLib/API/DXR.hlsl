// Note: TextureObject.Sample is not supported for DXR shader so we used SampleLevel instead

#if SHADER_API_D3D11 && SHADER_STAGE_RAYTRACING
// This file assume SHADER_API_D3D11 is defined

struct TextureSampleGrad
{
    float2 duv_dx;
    float2 duv_dy;
};
static TextureSampleGrad s_DXRTextureSampleGrad = { float2(0, 0), float2(0, 0) };
...
#define SAMPLE_TEXTURE2D(textureName, samplerName, coord2)                               textureName.SampleGrad(samplerName, coord2, s_DXRTextureSampleGrad.duv_dx, s_DXRTextureSampleGrad.duv_dy)
